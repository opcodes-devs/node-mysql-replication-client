                          ...
                          )
{
  if (!useDateStringsForType(dateStrings, 'DATE')) {
    const momentDate = moment.tz(`${year}-${jsMonthFromMysqlMonth(month)}-${date}`, timezone);
                              ...
                              )
{
  if (!useDateStringsForType(dateStrings, 'DATETIME')) {
    const momentDate = moment.tz(`${year}-${jsMonthFromMysqlMonth(month)}-${date} ${hour}:${minute}:${second}${fraction ? `.${fraction.milliseconds}` : ''}`, timezone);
    return momentDate.toDate();
  }
  return getDateString(year, month, date) + ' ' +
